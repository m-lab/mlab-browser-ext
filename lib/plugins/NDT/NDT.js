...
NDT.prototype.init = function (params) {
  var prefs = new TestPrefs.TestPrefs();
  var ns = new MlabNS.MlabNS();
  var mlabnsPref = prefs.getPreference("NDT", "useMlabNS");

  if (mlabnsPref == null) {
    mlabnsPref = false;
    preferences = this.getPreferences();
    for (i in preferences) {
      if (preferences[i].key == "useMlabNS") {
        mlabnsPref = (preferences[i].defaultValue == "true") ? true : false;
        break;
      }
    }
    console.error("Using default value of useMlabNS: " + mlabnsPref);
  }

  if (mlabnsPref) {
    console.error("Using mlabns.");
    this.serverName = ns.resolve();
  }

  if (this.serverName == null) {
    this.serverName = params[0];
  }
  console.error("Using server: " + this.serverName);
...
NDT.prototype.runTest = function (done) {
  this.done = done;
  this.connectTimeout = Timers.setTimeout(this.cancelTestConnection,
    Constants.NDTProtocolConstants.CONNECT_TIMEOUT,
    this);
  this.open();
}
...
NDT.prototype.cancelTestConnection = function (state) {
  state.results["error"] = "Timeout connecting to MLab testing server (" +
    state.serverName +
    ").";
...
NDT.prototype.nextTest = function () {
  ...
  console.error("Started to run " + this.runningTest.toString());
}
...
NDT.prototype.onTransportStatus = function(transport,status,progress,progressMax) {
  ...
  if (status == Ci.nsISocketTransport.STATUS_RESOLVING) {
    ...
    console.error("Connected. Canceling connection timeout timer.");
...
NDT.prototype.getPreferences = function () {
  return [
    {
      type: "boolean",
      key: "useMlabNS",
      defaultValue: "true",
      description: "Use Mlab NS?"
    }
    ];
}

NDT.prototype.getDescription = function () {
  return "The Network Diagnostic Tool (NDT) runs tests between your computer and the nearest M-Lab server, performing diagnostic functions and measuring the overall health of your current connection. For more information, see: <a href='http://www.measurementlab.net/tools/ndt'>http://www.measurementlab.net/tools/ndt</a>.";
}

NDT.prototype.getTitle = function () {
  return "Network Diagnostic Test";
}

NDT.prototype.getDuration = function () {
  return 30;
}
