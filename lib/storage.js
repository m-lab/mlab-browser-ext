 ...
 * this time.
 * @param {int} [max] Only retrieve max number of
 * tests.
 *
 * @returns {number[]} The times of the tests for
 * which results are available.
 */
Storage.prototype.listResults = function (test, after, before, max) {
  var statementString = "SELECT time from mlab where test=:test";
  var numResults = 0;
  var results = [];
  console.error("Listing test results.");
  ...
  statementString += " order by time desc";
 ...
 */
Storage.prototype.getResults = function (test, after, before, max) {
