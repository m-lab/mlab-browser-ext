...
Mlab.prototype.setWorker = function (worker) {
  ...
  this.worker.port.on("getTestDescription", this.getTestDescription.bind(this));
  this.worker.port.on("getTestTitle", this.getTestTitle.bind(this));
  this.worker.port.on("setVisibleTest", this.setVisibleTest.bind(this));
  this.worker.port.on("getTestDuration", this.getTestDuration.bind(this));
}

Mlab.prototype.getTestDuration = function (testName) {
  var test = this.getTestByName(testName);
  if (!test) {
    return 0;
  }
  if (this.worker != null) {
    console.error("testDuration: " + test.getDuration());
    this.worker.port.emit("testDuration", {
      name: testName,
...
Mlab.prototype.setVisibleTest = function (visibleTest) {
  if (this.ui != null) {
    this.ui.setVisibleTest(visibleTest);
  }
}
 ...
 */
Mlab.prototype.listTestResults = function (testName, after, before, max) {
  this.worker.port.emit(testName + ".testResultsList",
    {
      test: testName,
      results: Storage.getStorage().listResults(testName, after, before, max)
    });
}
 ...
 */
Mlab.prototype.getTestResults = function (testName, after, before, max) {
  this.worker.port.emit(testName + ".testResults",
    {
      test: testName,
      results: Storage.getStorage().getResults(testName, after, before, max)
    });
}
...
Mlab.prototype.startTest = function (testName) {
  ...
  for (i in this.tests) {
    if (this.tests[i].name == testName) {
      parameters = this.tests[i].parameters;
      break;
    }
  }

  this.worker.port.emit("testStarted", testName);
...
Mlab.prototype.getTestTitle = function (testName) {
  var test = this.getTestByName(testName);
  if (test == null) {
    return null;
  }
  if (this.worker != null) {
    this.worker.port.emit("testTitle", test.getTitle());
  }
  return test.getTitle();
}

Mlab.prototype.getTestDescription = function (testName) {
  var test = this.getTestByName(testName);
  if (test == null) {
    return null;
  }
  if (this.worker != null) {
    this.worker.port.emit("testDescription", test.getDescription());
  }
...
Mlab.prototype.getTestByName = function (testName) {
  var test = null;
  for (i in this.tests) {
    if (this.tests[i].name === testName) {
      var testClass = require("plugins/" +
