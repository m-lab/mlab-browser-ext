 ...
 * Initialize?
 */
mlab.forEachTest(function (t) {
  testPreferences = t.getPreferences();
  for (i in testPreferences) {
    if (testPreferences[i].type == "boolean") {
      if (Preferences.prefs[testPreferences[i].key] == null) {
        console.error("Setting a new preference!");
        Preferences.prefs[testPreferences[i].key] = true;
      }
    }
  }
});
